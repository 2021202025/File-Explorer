...
void printFiles()
{
    ...
    for (int it = top; it < bottom && it < n; it++)
    {
        ...
        for (auto i = 0; i < dt.size() - 1; i++)
            // s = s + dt[i];
            cout << dt[i];

        // display's file or directory name
        // s = s + "\t" + fileContents[it]->d_name;
        // s = s + "\n";
        string name = fileContents[it]->d_name;

        if (colRight <= 88 or colDown < 30)
        {
            cout << "\e[8;30;100t";
            colRight = 100;
            ...
            else if (ss >= colRight - 3)
            {
                if ((S_ISDIR(fileInfo.st_mode)))
                {
                    cout << "\t"
                         << "\033[1;94m" << name.substr(0, colRight - 78) << "..."
                         << "\033[0m";
                }
                ...
                {
                    cout << "\t"
                         << "\033[1;97m" << name.substr(0, colRight - 78) << "..."
                         << "\033[0m";
                }
            }
...
void setDirectory(char const *dirPath)
{
    ...
    colRight = win.ws_col;
    colDown = win.ws_row;
    commandLine = win.ws_row - 1;
    statusLine = commandLine + 1;
    redLine = commandLine - 2;
    cursor = 0;
...
string commandInput()
{
    ...
    while (true)
    {
        ...
        // else if (ch == 'q' and commandCursorLoc == 14)
        // {
        //     exit(0);
        // }
        else if (ch == 10)
